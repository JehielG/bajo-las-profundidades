class Juyos1S extends Phaser.Scene{ 
    constructor(){ 
        super({ 
            key: 'Juyos1S' 
        }); 
    ...
    preload() {
		...
		this.load.image(['tablero_borde-abajo', 'cortina', 'tablero', 'fondo-mina']);
  ...
  //       // this.sys.animatedTiles.init(this.mapaLava);
		...
		this.fondo = this.add.image(0, 0, 'fondo-mina').setOrigin(0,0);
		...
		this.plataforma.body.setAllowGravity(false);
      	...
      	this.megaPiedra = this.add.sprite(913,445,"mega_piedra").setScale(3);
		this.megaPiedra.anims.play("mega_piedra_neutro");
...
this.physics.add.collider(this.juyosArray,this.plataforma);
		...
		if(this.juyosArray.length==20){
			// Cambia a la siguiente escena
			if(this.bandCambio){
				console.log("Pasa a la escena de BaseS antes Cueva2S");
				// sig
				// this.scene.start('BaseS',this.totalVidas);
				this.scene.start('Cueva2S', 2);
